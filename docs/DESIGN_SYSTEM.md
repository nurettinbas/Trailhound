...
| `YearRecapScenes.swift` | Recap story Canvas. Intro is a title card (vanishing road + horizon bloom, dust motes; no flag / plate / corner sun). Copy: tracked kicker, 80pt rounded year with palette glow shadow, whisper — `glassPrimaryInk` / `glassSecondaryInk`, no extra glass host. |
| `RecapShareRenderer.swift` | Year recap Share PNG is `ImageRenderer` of the visible story page (`RecapPageScene` + copy), 9:16. Not a separate Core Graphics poster. Intro is a title card: kicker + hero year + whisper on a vanishing road with horizon bloom — no flag, plate, or corner sun. |
