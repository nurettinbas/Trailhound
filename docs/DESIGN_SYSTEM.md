...
| Tab selection | System iOS 26 `UITabBar`. Light is system glass; **selected** icon + title are **white**. Dark: system glass, white unselected, palette selected. Native press + pill morph. Do not copy `UITabBarAppearance`. | Dark: system glass, white unselected, palette selected |
...
| `StatsCard.swift` | Stats full/half cards (native Light `glassEffect`), nested tiles, `.statsNestedPanel()` (variable-height frost in expand overlays), `.statsFrostChip()`, `StatsSegmentBar` / `StatsSegmentSwatch` / `StatsShareBar` (vehicle-compare share of top spend, donut slice fill), overlay `posterHeight` / `posterExpandedHeight`. Recap and month-forecast posters use `contentInset: 0` plus `.statsPosterOverlayPadding()` so artwork fills the card. Forecast copy matches Recap: title chip top-left, amount + metrics on a bottom atmosphere gradient — not a whitish sparkline veil. |
...
| `TrailhoundTabBarCompact.swift` | System `UITabBar`. Light selected is white. Dark palette tint. Do not copy `UITabBarAppearance` on iOS 26. |
