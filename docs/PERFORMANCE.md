...
- Form/list, Trip/Travel detail, and Vehicles detail **nav-bar** buttons use the **system** toolbar platter (same host as the Trips merge+bell cluster). Light iOS 26 does not apply white `.tint` or `toolbarColorScheme(.light)` (those paint a second plate). `.glassNavigationChrome()` hides the bar fill and the iOS 26 top/bottom `scrollEdgeEffect`. Trip/Travel maps also use `.glassMapTopEdgeHidden()`. The floating tab bar is SwiftUI `TrailhoundFloatingTabBar` + `.glassTabBar()` (one Capsule `.clear` host). The system `UITabBar` is hidden so Light vibrancy cannot chameleon-stamp glyphs. Unselected is white; selected icon + title are black. Custom back keeps the system chevron hidden; `NavigationInteractivePopEnabler` re-enables the edge-swipe pop (including over MapKit). Vehicles passes `disabled` while the embedded editor has unsaved edits so swipe cannot discard silently.
