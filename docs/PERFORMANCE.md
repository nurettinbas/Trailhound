...
- **Live follow map** (`LiveFollowMapView` + `LiveFollowMapKitView`): `CADisplayLink` (capped at 60 fps) drives `LiveFollowCamera`. The same `lastLocation` is not re-ingested every frame (that reset the dead-reckon clock and snapped at ~1 Hz). Camera center is the vehicle (no look-ahead) so the puck sits at screen center while following. Motion is **velocity integration** (`speed × dt`) between GPS samples — implied speed/path bearing when Core Location reports `-1` (common in Simulator) — so the map does not hitch at 1 Hz. Two rules keep that continuous: the fresh window (`maxDeadReckonSeconds`) is well above the ~1 Hz fix interval, and once a fix *is* stale the coast **decays** (`deadReckonDecayTauSeconds`) instead of zeroing speed in one frame. Blending back toward the last GPS speed is skipped during that decay, otherwise the two fight and settle at a constant coast. GPS is only a trim on the integrated position: lateral and along-track corrections use long time constants, along-track pushes only when the fix is *ahead*, and a spent coast on a stale fix **holds** rather than rewinding. `LiveFollowSession.camera` is observation-ignored so 60 fps ticks do not rebuild SwiftUI. Camera writes use `map.camera` inside `CATransaction` (actions disabled) **only while following**; after pan, pose/puck keep updating. Follow tiles stay `.flat` with pitch for 3D feel. The traveled path is an `MKMultiPolyline` history (rebuilt only when a breadcrumb lands, ~1 Hz) plus a **two-point `MKPolyline` tip** from the last breadcrumb to the puck, swapped at ~12 Hz — one solid blue stroke. Both use MapKit's *own* vector renderers on purpose: a custom `MKOverlayRenderer` is rasterised flat and then perspective-warped, which fattens the stroke toward the bottom of a 3D view, seams it while panning a pitched camera, and re-rasterises world-sized tiles on every invalidate. A tip longer than 120 m is dropped (GPS gap). Pan/zoom stay enabled while paused. **Show entire route** fits start + trail + puck, north-up 2D, then leaves the camera free. `ScreenIdleLock` keeps the display awake only while the cover is open. Start / pause / trip-stop pins render on the live map. While the cover is open, the card’s road `TimelineView` stays paused (`!showLiveFollowMap`).
