...
Inserted **between** the goal+hero pair and the summary grid. Same `.statsFullCard()` chrome as the rest of the tab. They do **not** replace year awards.

| Card | Role |
|---|---|
| **Year recap** | YTD hub + Play. Full-screen story is a cover, not a list card. Autoplay once in December/January. Unfiltered year, independent of Stats chips. |
| **Badges** | Horizontal strip → gallery. In-app only (no widget). Unlock overlay on Stats. |
| **Frequent routes** | Top corridor + MapKit snapshot thumbnail → full-screen arcs/heatmap (40-corridor cap). Privacy-safe home/work labels. |
| **Month cost forecast** | Projected month-end total. Driving fuel estimate + installments + other expenses; **logged pump fuel is a separate line** and is never double-counted into the hero total. Independent of the comparison **Logged vehicle expenses** card. |

**Year awards** stay after the chart pagers (deferred loader). Recap and awards can both be on screen — recap is the story; awards are the medal grid.
...
Deep links `trailhound://stats/goal|forecast|recap|routes` open this tab and the matching sheet/cover.
