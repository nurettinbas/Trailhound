...
- **Liquid Glass** (iOS 26 native `glassEffect`, Material fallback on iOS 17/18): frosted cards over the selected hue so the atmosphere shows through. Light uses native glass with a mid-family tint — not a dark chrome plate or milky white card. The glass plate clips to the card; labels in the corners are not sliced. Reduce Transparency uses an opaque mid-family fill. Dark is a deep shade of the same color. The live recording card, follow path, and vehicle puck use the same hue; **Stop** stays solid red. The tab bar is the system iOS 26 Liquid Glass bar. Light uses the default system bar with a white selected tab; Dark keeps unselected white and a palette selected tint. Map toolbars stay frozen circles so MapKit is not resampled
...
- **Badges**: cumulative km (100 silver, 1,000 gold, 10,000 teal platinum, 100,000 lavender diamond — each km medal runs a different motion on the S; the full km ladder stays in the gallery even while locked), plus business / streak / cities / night / corridor ladders and trips, hours, long haul, dawn, weekend, fleet, and countries — each family has its own color, icon, and idle. Stats strip shows unlocked medals only; expand is a plain toolbar glyph on the card, then a circled collapse once expanded; the card grows into a glass gallery with 3D round medals (unlocked first); locked badges show formatted progress (km or counts) without spoiling the next non-km tier. Share on an unlocked medal is a compact palette-tint chip and exports a 9:16 story card (atmosphere, Trailhound mark, glass plate, 3D medal + copy) for Instagram / WhatsApp stories or Messages — not a title string. Newly unlocked badges celebrate on Stats (3 s each, then the next) and also land in the notifications inbox with a local banner (tap opens the gallery)
- **Frequent-routes map** (Stats card expands as the MapKit map itself — road polylines + heatmap; privacy-safe home/work labels; habit corridors in the same city as your top run)
