...
- **Liquid Glass** (iOS 26 native `glassEffect`, Material fallback on iOS 17/18): frosted cards over the selected hue so the atmosphere shows through. Light uses native glass with a mid-family tint — not a dark chrome plate or milky white card. The glass plate clips to the card; labels in the corners are not sliced. Reduce Transparency uses an opaque mid-family fill. Dark is a deep shade of the same color. The live recording card, follow path, and vehicle puck use the same hue; **Stop** stays solid red. The floating tab bar is clear Liquid Glass; unselected icons are white and the selected tab’s icon and title are black. Map toolbars stay frozen circles so MapKit is not resampled
