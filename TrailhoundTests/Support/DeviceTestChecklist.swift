...
enum DeviceTestChecklist {
    static let items = [
        ...
        "Live follow map: pan/recenter → pause/stop; 3D/2D toggle; overview fits start+trail; single annotation puck stays visible after pan (no second pointer, never disappears)",
        "Live follow map: smooth camera on straight road and turns (continuous between GPS fixes, no 1 Hz hitch / no rewind)",
        "Live follow map: single draped blue path (MKMultiPolyline history + two-point tip polyline) grows with the puck; no duplicate / X crossover; real GPS gaps only",
